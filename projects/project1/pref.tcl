...
#
# Define some commands so this file can be sourced by other programs, and
# to avoid errors during the pkg_mkIndex step
#
...
if {[info commands IsQuesta] eq ""} {
	proc IsQuesta {} {return 1}
}
if {[info commands IsSystemCPlatform] eq ""} {
   proc IsSystemCPlatform {} {return 0}
}
...
#
if {[info commands vsimcmd::AccNameCompositesList] eq "vsimcmd::AccNameCompositesList"} {
	set WildcardFilter [vsimcmd::AccNameCompositesList Default]
} else {
	set WildcardFilter [list Variable Constant Generic Parameter SpecParam Memory Assertion Cover Endpoint CellInternal ImmediateAssert ScVariable]
}
...
set pname [ProductPrompt]
append pname "> "
set PrefMain(prompt2) $pname
...
set PrefMain(HyperLinkingEnabled) 0
set PrefMain(HyperLinkingUnderline) 0
...
# Control flags for SelectedSignal cross highlighting
set PrefMain(CrossSelectExpandBus) 0
set PrefMain(CrossSelectEnabled) 1
set PrefMain(FocusFollowsMouse) 0
set PrefMain(FFMDelay) 300  ;# Focus-Follows-Mouse: delay before focus takes effect

# Some DnD, toolbar, and menu operations will optionally transcribe a command based
# the this control:
set PrefMain(ShowCommands) all ;# legal values: all, none, hist[ory], trans[cript]
...
# 	set PrefUCDB(font)            $PrefDefault(textFont)
	...
	set PrefDefault(searchbarFiltered) "#ffd700" ;# gold
	...
	set PrefDataflow(sproutlimit)   10
	set PrefDataflow(p2plimit)      400
	set PrefDataflow(showhierarchy) 0
	set PrefDataflow(showacttime)   0
	set PrefDataflow(stoponport)    0
	set PrefDataflow(hidebuffers)   0
	set PrefDataflow(hideinverters) 0
	set PrefDataflow(autosource)    1
	set PrefDataflow(symlibs)       {}
	set PrefDataflow(popupenabled)  1
    # show dataflow nets as dashed when incomplete
	set PrefDataflow(shownetstate)   1

   # Beginning in 6.6, PrefSchematic holds settings for the new incremental/full
   # schematic window (formerly these were stored in PrefDataflow). One difference
   # in the PrefSchematic array is that the "nosprout" option was removed since it
   # was tied to some old behavior we won't be carrying forward. Another difference
   # is the "showhierarchy" option is ON by default.
   #
	set PrefSchematic(mouseMode)     Select
	set PrefSchematic(hidecells)     1
	set PrefSchematic(inoutlocation) 1
	set PrefSchematic(keepcontent)   1
	set PrefSchematic(lognets)       1
	set PrefSchematic(autowave)      1
	set PrefSchematic(selectenv)     1
	set PrefSchematic(selequivnets)  0
	set PrefSchematic(showvalues)    1
	set PrefSchematic(valuelength)   16
	set PrefSchematic(outputquerylimit) 100
	set PrefSchematic(p2plimit)      400
	set PrefSchematic(maxbussize)    5000
	set PrefSchematic(showhierarchy) 1
	set PrefSchematic(showacttime)   1
	set PrefSchematic(hidebuffers)   0
	set PrefSchematic(hideinverters) 0
	set PrefSchematic(autosource)    1
	set PrefSchematic(symlibs)       {}
	set PrefSchematic(popupenabled)  1
	set PrefSchematic(showdunames)   0
	set PrefSchematic(showinstnames) 1
	set PrefSchematic(shownetnames)  0
	set PrefSchematic(shownetnamestyle) corner ;# choices are "corner" or "edge"
	set PrefSchematic(showpinnames)  1
	set PrefSchematic(showripindices) 1
	set PrefSchematic(mouseoverenabled) 1
	set PrefSchematic(verticaltext) 1
	set PrefSchematic(1clickmode) 0
	# Controls if control logic should be followed when doing an Expand To...
	set PrefSchematic(netxFollowsControlLogic) 0
	# Expand To... control when to give up on large fan-in/out
	#   Stop after passing through N gates, or through N hierarchy levels
	set PrefSchematic(netxMaxGates) 1024
	set PrefSchematic(netxMaxLevels) 32
    # we don't show schematic nets as dashed when incomplete, yet
	set PrefSchematic(shownetstate)   0
...
#	set PrinterSetup(dffont) Helvetica
  ...
  set PrefSource(HighlightErrorColor) orange
	...
	set PrefSource(RunUntilHereUseRL) 0
	if {$tcl_platform(platform) == "windows"} {
	...
	set PrefSource(ShowLoadDebugMessage) 1
	set PrefSource(NoDBS) 0
	...
	set PrefWave(doubleClick) "active" ;# for 6.5 and earlier this was ShowDriversEnabled
	...
	set PrefWave(gridEnable) on
	set PrefWave(timeline)   0
	set PrefWave(GroupName) "Group#"
	set PrefWave(OpenLogAutoAddWave) 0
	set PrefWave(EnablePAHighlight) 0
	set PrefWave(ShowArraySize) 0
	set PrefWave(InsertMode) insert ;# legal values: insert, apped, end, top

	set PrefSignals(nameColWidth) 150
	set PrefSignals(valueJustify) left
	set PrefSignals(ShowArraySize) 0

	set PrefClassInstances(nameColWidth) 150
	set PrefClassInstances(valueJustify) left
	set PrefClassInstances(ShowArraySize) 0

	set PrefStructure(autoExpand) 1
	set PrefStructure(accFilterV2) {AllScopes AllProcesses}
	set PrefStructure(expandAllTops) 1
	...
	if {[winfo depth .] > 1} {
		...
		set PrefClassInstances(background) $PrefMain(background) 
		set PrefClassInstances(foreground) $PrefMain(foreground) 
		set PrefClassInstances(selectBackground) $PrefDefault(select_bg)
		set PrefClassInstances(selectForeground) $PrefDefault(select_fg)
		set PrefOVM(background) #f8f8ff
		set PrefOVM(foreground) Black
		set PrefOVM(Matchbackground) #98fb98
		set PrefOVM(NoMatchbackground) #ffe4e1
		...
		set PrefWave(waveStripBackground) grey20
		set PrefWave(waveDeltaBackground) MidnightBlue
		set PrefWave(waveEventBackground) DarkGreen
		...
		set PrefDataflow(changeColor) "#ff5500"
		set PrefDataflow(foldColor) "#004080"
		set PrefDataflow(highlightColor1) red
		set PrefDataflow(highlightColor2) green
		set PrefDataflow(highlightColor3) magenta
		set PrefDataflow(highlightColor4) gold
		set PrefDataflow(highlightColor5) blue
		set PrefDataflow(highlightLabel1) "Highlight1"
		set PrefDataflow(highlightLabel2) "Highlight2"
		set PrefDataflow(highlightLabel3) "Highlight3"
		set PrefDataflow(highlightLabel4) "Highlight4"
		set PrefDataflow(highlightLabel5) "Highlight5"
		set PrefDataflow(selectColor) salmon

		set PrefSchematic(background) Black
		set PrefSchematic(textColor) White
		set PrefSchematic(pinColor) Cyan
		set PrefSchematic(netColor) Yellow
		set PrefSchematic(hierboxColor) grey60
		set PrefSchematic(hierfillColor) "#421031"
		set PrefSchematic(instColor) Cyan
		set PrefSchematic(valueColor) White
		set PrefSchematic(changeColor) "#ff5500"
		set PrefSchematic(foldColor) "#000084"
		set PrefSchematic(selectColor) salmon
		set PrefSchematic(strokeColor) "orange red"
		set PrefSchematic(highlightColor1) red
		set PrefSchematic(highlightColor2) green
		set PrefSchematic(highlightColor3) magenta
		set PrefSchematic(highlightColor4) gold
		set PrefSchematic(highlightColor5) blue
		set PrefSchematic(mouseoverColor)  magenta
		set PrefSchematic(highlightLabel1) "Highlight1"
		set PrefSchematic(highlightLabel2) "Highlight2"
		set PrefSchematic(highlightLabel3) "Highlight3"
		set PrefSchematic(highlightLabel4) "Highlight4"
		set PrefSchematic(highlightLabel5) "Highlight5"
		...
		set PrefUCDB(selectBackground) $PrefDefault(select_bg)
		set PrefUCDB(selectForeground) $PrefDefault(select_fg)
	} else {
		# Monochrome defaults
		...
		set PrefClassInstances(background) White
		set PrefClassInstances(foreground) Black
		set PrefClassInstances(selectBackground) Black
		set PrefClassInstances(selectForeground) White
		set PrefOVM(background) White
		set PrefOVM(foreground) Black
		set PrefOVM(Matchbackground) grey20
		set PrefOVM(NoMatchbackground) grey50
		...
		set PrefWave(selectForeground) Black
		set PrefWave(waveBackground) Black
		set PrefWave(waveStripBackground) Black
		set PrefWave(waveDeltaBackground) Black
		set PrefWave(waveEventBackground) Black
		set PrefWave(waveForeground) White
		...
		set PrefDataflow(changeColor) White
		set PrefDataflow(foldColor) grey20

		set PrefSchematic(background) Black
		set PrefSchematic(textColor) White
		set PrefSchematic(pinColor) White
		set PrefSchematic(netColor) White
		set PrefSchematic(hierboxColor) White
		set PrefSchematic(hierfillColor) grey50
		set PrefSchematic(instColor) White
		set PrefSchematic(valueColor) White
		set PrefSchematic(changeColor) White
		set PrefSchematic(foldColor) grey20
		set PrefSchematic(rubberbandColor) grey50
		set PrefSchematic(selectColor) grey50
		set PrefSchematic(mouseoverColor) White
		...
		set PrefUCDB(background) White
		set PrefUCDB(foreground) Black
		set PrefUCDB(selectBackground) White
		set PrefUCDB(selectForeground) Black
	}
	...
	set PrefClassInstances(background2)   [Color::AdjustLuminance $PrefClassInstances(background)   -0.05]
	set PrefOVM(background2)       [Color::AdjustLuminance $PrefOVM(background)		  -0.05]
...
set PrefSource(VRMFiles)     {{Project Files}  {.vrm}                                                    }
set PrefSource(IniFiles)     {{Ini Files}      {.ini}                                                    }
set PrefSource(AllFiles)     {{All Files}      *                                                         }
set PrefSource(UCDBFiles)    {{UCDB Files}     {.ucdb}                                                   }
set PrefSource(PowerFiles)   {{Power Files}    {.upf .pcf}                                               }
set PrefSource(UPFFiles)     {{UPF Files}      {.upf}                                                    }
set PrefSource(PCFFiles)     {{PCF Files}      {.pcf}                                                    }
set PrefSource(RankFiles)    {{Rank Files}     {.rank}                                                   }
set PrefSource(DatasetFiles) {{Dataset Files}  {.wlf *.ucdb}                                             }
set PrefSource(RMDBFiles)    {{RunMgr Db Files} {.rmdb .tcl}                                             }
...
set PrefSource(HTMLFiles)    {{HTML Files}     {*.html *.htm}                                            }
set PrefSource(DebugArchiveFiles) {{Debug Archive Files} {*.dbar}                                        }
set PrefSource(DebugDatabaseFiles) {{Debug Database Files} {*.dbg}                                       }
...
#--- VERILOG ---
lappend PrefSource(verilogFileTypes) \$PrefSource(verilogFiles)
lappend PrefSource(verilogFileTypes) \$PrefSource(systemVerilogFiles)
lappend PrefSource(verilogFileTypes) \$PrefSource(VHDLFiles)
lappend PrefSource(verilogFileTypes) \$PrefSource(PSLFiles)
lappend PrefSource(verilogFileTypes) \$PrefSource(HDLFiles)
lappend PrefSource(verilogFileTypes) \$PrefSource(GZFiles)
lappend PrefSource(verilogFileTypes) \$PrefSource(AllFiles)

#--- SYSTEMVERILOG ---
lappend PrefSource(systemVerilogFileTypes) \$PrefSource(systemVerilogFiles)
lappend PrefSource(systemVerilogFileTypes) \$PrefSource(verilogFiles)
lappend PrefSource(systemVerilogFileTypes) \$PrefSource(VHDLFiles)
lappend PrefSource(systemVerilogFileTypes) \$PrefSource(PSLFiles)
lappend PrefSource(systemVerilogFileTypes) \$PrefSource(HDLFiles)
lappend PrefSource(systemVerilogFileTypes) \$PrefSource(GZFiles)
lappend PrefSource(systemVerilogFileTypes) \$PrefSource(AllFiles)

#--- VHDL ---
lappend PrefSource(vhdlFileTypes) \$PrefSource(VHDLFiles)
lappend PrefSource(vhdlFileTypes) \$PrefSource(verilogFiles)
lappend PrefSource(vhdlFileTypes) \$PrefSource(PSLFiles)
lappend PrefSource(vhdlFileTypes) \$PrefSource(HDLFiles)
lappend PrefSource(vhdlFileTypes) \$PrefSource(GZFiles)
lappend PrefSource(vhdlFileTypes) \$PrefSource(AllFiles)

#--- SYSTEM C ---
lappend PrefSource(systemcFileTypes) \$PrefSource(CFiles)
lappend PrefSource(systemcFileTypes) \$PrefSource(AllFiles)

#--- PSL ---
lappend PrefSource(pslFileTypes) \$PrefSource(PSLFiles)
lappend PrefSource(pslFileTypes) \$PrefSource(AllFiles)

#--- HDL ---
lappend PrefSource(HDLFileTypes) \$PrefSource(HDLFiles)
lappend PrefSource(HDLFileTypes) \$PrefSource(VHDLFiles)
lappend PrefSource(HDLFileTypes) \$PrefSource(verilogFiles)
lappend PrefSource(HDLFileTypes) \$PrefSource(systemVerilogFiles)
lappend PrefSource(HDLFileTypes) \$PrefSource(PSLFiles)
lappend PrefSource(HDLFileTypes) \$PrefSource(GZFiles)
lappend PrefSource(HDLFileTypes) \$PrefSource(AllFiles)

#--- HDL + C ---
lappend PrefSource(HDLCFileTypes) \$PrefSource(HDLFiles)
lappend PrefSource(HDLCFileTypes) \$PrefSource(VHDLFiles)
lappend PrefSource(HDLCFileTypes) \$PrefSource(verilogFiles)
lappend PrefSource(HDLCFileTypes) \$PrefSource(systemVerilogFiles)
lappend PrefSource(HDLCFileTypes) \$PrefSource(PSLFiles)
if {[IsSystemCPlatform]} {
lappend PrefSource(HDLCFileTypes) \$PrefSource(CFiles)
}
lappend PrefSource(HDLCFileTypes) \$PrefSource(GZFiles)
lappend PrefSource(HDLCFileTypes) \$PrefSource(AllFiles)


set PrefSource(AllFileTypes) { $PrefSource(AllFiles) }
...
set PrefMain(VRMFileTypes) {{{VRM Files} {.vrm}} {{All Files} *}}
...
set PrefMain(HTMLFileTypes) \
	{\
		 $PrefSource(HTMLFiles) \
		 $PrefSource(AllFiles) \
	 }
set PrefMain(DebugArchiveFileTypes) [list $PrefSource(DebugArchiveFiles) $PrefSource(AllFiles)]
set PrefMain(DebugDatabaseFileTypes) [list $PrefSource(DebugDatabaseFiles) $PrefSource(AllFiles)]

set PrefMain(AllFileTypes) [list $PrefSource(AllFiles)]

#--- GENERAL ---
lappend PrefMain(GeneralFileTypes) \$PrefSource(HDLFiles)
lappend PrefMain(GeneralFileTypes) \$PrefSource(CFiles)
lappend PrefMain(GeneralFileTypes) \$PrefSource(LogFiles)
lappend PrefMain(GeneralFileTypes) \$PrefSource(ProjectFiles)
lappend PrefMain(GeneralFileTypes) \$PrefSource(GZFiles)
lappend PrefMain(GeneralFileTypes) \$PrefSource(MacroFiles)
lappend PrefMain(GeneralFileTypes) \$PrefSource(verilogFiles)
lappend PrefMain(GeneralFileTypes) \$PrefSource(systemVerilogFiles)
lappend PrefMain(GeneralFileTypes) \$PrefSource(VHDLFiles)
lappend PrefMain(GeneralFileTypes) \$PrefSource(PSLFiles)
lappend PrefMain(GeneralFileTypes) \$PrefSource(SDFFiles)
lappend PrefMain(GeneralFileTypes) \$PrefSource(TextFiles)
lappend PrefMain(GeneralFileTypes) \$PrefSource(DoFiles)
lappend PrefMain(GeneralFileTypes) \$PrefSource(TCLFiles)
lappend PrefMain(GeneralFileTypes) \$PrefSource(UCDBFiles)
lappend PrefMain(GeneralFileTypes) \$PrefSource(PowerFiles)
lappend PrefMain(GeneralFileTypes) \$PrefSource(RMDBFiles)
lappend PrefMain(GeneralFileTypes) \$PrefSource(HTMLFiles)
lappend PrefMain(GeneralFileTypes) \$PrefSource(AllFiles)
...
set PrefCompare(defaultCollapseTime) 0
...
set PrefMemory(ShowVHDLStringAsMem) 0
